...
On Windows, ensure that your gcc is Clang-compatible. You can set up the environment using msys2 and install the necessary tools:

1. Install msys2 from [msys2.org](https://www.msys2.org/).
2. Open the MSYS2 terminal and run the following commands:
   ```bash
   pacman -Syu
   pacman -S mingw-w64-clang-x86_64-gcc-compat mingw-w64-clang-x86_64-clang make
   ```
3. Add the msys2 and Clang tool paths to your system PATH.

If the build process detects an incompatible compiler, it will throw an error. Please ensure your environment is set up correctly as per the instructions above.
### MacOS
...
Additionally, ensure that your GCC is Clang-compatible. If not, the build process will throw an error. Follow the setup instructions for installing and verifying the correct compiler environment using MSYS2. You can install the necessary tools with:

```
pacman -S mingw-w64-clang-x86_64-gcc-compat mingw-w64-clang-x86_64-clang make
```

Ensure that the paths for MSYS2 and Clang tools are appended to the system path to make them available during the build process.
