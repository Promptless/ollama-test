...
- gcc version 11.4.0 or higher (on Windows, ensure gcc is a Clang-compatible compiler. See below for setup instructions.)
...
Ensure that the GCC version detected is Clang-compatible to avoid build errors.
### MacOS
```bash
...
**Important:** Ensure that GCC is a Clang-compatible compiler on Windows. If not, follow the setup instructions below to install the necessary tools using MSYS2.
