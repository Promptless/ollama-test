...
### Advanced `Modelfile` with Multimodal Support

An example of a `Modelfile` utilizing the new "mllama" architecture with image processing capabilities:

```modelfile
FROM mllama
# sets the temperature to 0.7 for balanced creativity and coherence
PARAMETER temperature 0.7
# sets the context window size to 2048, optimized for multimodal models
PARAMETER num_ctx 2048

# sets a custom system message to specify the behavior of the chat assistant
SYSTEM You are an AI assistant capable of processing both text and images.
```
...
2. `ollama create choose-a-model-name -f <location of the file e.g. ./Modelfile>'`
3. `ollama run choose-a-model-name`
4. Start using the model with both text and image inputs!

This example demonstrates how to leverage the new "mllama" architecture for enhanced multimodal interactions.
## Instructions
